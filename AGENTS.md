...
このリポジトリは Neovim 用 Lua プラグインのテンプレートです。初期状態では Lua モジュール名が `template` になっています。

## ディレクトリ構成

- `lua/template/`: プラグイン本体。テンプレートから作成した後は `lua/<PLUGIN_NAME>/` にリネームされます。
...
- テンプレートのプレースホルダー `template` を変更する場合は、手作業で一部だけ置換せず `make rename PLUGIN_NAME=<lowercase_lua_module_name>` を使います。
